...
def mk_multi_branch(tree, indices):
    ...
    while not complete:
        ...
        for k in keys:
            if (
                k * 2 in calculable_indices
                and (k * 2) + 1 in calculable_indices
                and k not in calculable_indices
            ):
    ...
    for i, b in zip(indices, output):
        ...
        for j in range(1, len(b)):
            if (
                (
                    (index ^ 1) * 2 in calculable_indices
                    and (index ^ 1) * 2 + 1 in calculable_indices
                )
                or ((index ^ 1) - len(tree) // 2 in indices)
                or (index ^ 1) in scanned
            ):
                b[j] = b""
...
def verify_multi_branch(root, indices, proof):
    ...
    while not complete:
        ...
        for k in keys:
            if (
                k * 2 in partial_tree
                and k * 2 + 1 in partial_tree
                and k not in partial_tree
            ):
