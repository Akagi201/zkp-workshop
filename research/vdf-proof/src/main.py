...
if encode_type == "vde":
    ...
    MIMC_constants = [
        (i**7) ^ 42 for i in range(min(N_trace // 2, 64))
    ]  # MiMC round constants
    # encoder and decoder (note that the choice of encoding key may suffer from grinding attack)
    encode = lambda d, _, addr, idx: mimc_encode(
        (d + addr + idx) % modulus, N_trace, MIMC_constants
    )
    decode = (
        lambda d, _, addr, idx: (mimc_decode(d, N_trace, MIMC_constants) - addr - idx)
        % modulus
    )
    mk_proof = lambda encoded_sample: mk_mimc_proof(
        encoded_sample, N_trace, MIMC_constants
    )
    verify_proof = (
        lambda encoded_sample, raw_sample, _, addr, idx, proof: verify_mimc_proof(
            encoded_sample,
            N_trace,
            MIMC_constants,
            (raw_sample + addr + idx) % modulus,
            proof,
        )
    )
    ...
    encode = (
        lambda d, h, addr, idx: (
            d
            + int.from_bytes(
                hash_fn(h + addr.to_bytes(20) + idx.to_bytes(8)), byteorder="big"
            )
        )
        % modulus
    )
...
def verify(N_s, seed, addr, diff, hashes, proof):
    ...
    for i in range(N_s):
        assert verify_proof(
            encoded_samples[i], samples[i], hashes[i], addr, pos[i], stark_proof[i]
        )
    ...
    # check integrity
    assert [hash_fn(sample.to_bytes(32, byteorder="big")) for sample in samples] == [
        hashes[p] for p in pos
    ]
...
def main(args=None):
    ...
    encoded_dataset = [
        encode(inp, hashes[i], addr, i) for (i, inp) in enumerate(dataset)
    ]
    print(
        "Encoding use time: %.2fs, avg : %.2f ms"
        % (time.time() - start_time, (time.time() - start_time) / N_d * 1000)
    )
    ...
    start_time = time.time()
    decoded_dataset = [
        decode(inp, hashes[i], addr, i) for (i, inp) in enumerate(encoded_dataset)
    ]
    assert decoded_dataset == dataset
    print(
        "Decoding use time: %.2fs, avg : %.2f ms"
        % (time.time() - start_time, (time.time() - start_time) / N_d * 1000)
    )
