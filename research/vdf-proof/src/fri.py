...
def prove_low_degree(
    values, root_of_unity, maxdeg_plus_1, modulus, exclude_multiples_of=0
):
...
def verify_low_degree_proof(
    merkle_root, root_of_unity, proof, maxdeg_plus_1, modulus, exclude_multiples_of=0
):
    f = PrimeField(modulus)
    ...
    for prf in proof[:-1]:
        ...
        # Compute the positions for the values in the polynomial
        poly_positions = sum(
            [[y + (roudeg // 4) * j for j in range(4)] for y in ys], []
        )
    ...
    poly = f.lagrange_interp(
        [powers[x] for x in pts[:maxdeg_plus_1]], [data[x] for x in pts[:maxdeg_plus_1]]
    )
