...
def inv_fft_at_point(vals, modulus, root_of_unity, x):
    ...
    comb = [
        (o * x * inv_root**i + e) % modulus
        for i, (o, e) in enumerate(zip(odds, evens))
    ]
    return inv_fft_at_point(
        comb[: len(comb) // 2], modulus, root_of_unity**2 % modulus, x**2 % modulus
    )
...
def shift_domain(vals, modulus, root_of_unity, factor):
    ...
    odds = [(f - g) * half % modulus for f, g in zip(vals, f_of_minus_x_vals)]
    print("o", odds)
    shifted_evens = shift_domain(
        evens[:half_length],
        modulus,
        root_of_unity**2 % modulus,
        factor**2 % modulus,
    )
    print("se", shifted_evens)
    shifted_odds = shift_domain(
        odds[:half_length], modulus, root_of_unity**2 % modulus, factor**2 % modulus
    )
