...
def mimc_encode(inp, steps, round_constants):
    for i in reversed(range(steps - 1)):
        inp = (
            pow(
                (inp - round_constants[i % len(round_constants)]) % modulus,
                (2 * modulus - 1) // 3,
                modulus,
            )
        ) % modulus
...
def mimc_decode(inp, steps, round_constants):
    for i in range(steps - 1):
        inp = (
            pow(inp, 3, modulus) + round_constants[i % len(round_constants)]
        ) % modulus
    return inp
...
def mk_mimc_proof(inp, steps, round_constants):
    ...
    constants_mini_extension = fft(
        constants_mini_polynomial, modulus, f.exp(G2, skips2)
    )
    ...
    l_evaluations = [
        (
            d_evaluations[i]
            + p_evaluations[i] * k1
            + p_evaluations[i] * k2 * powers[i]
            + b_evaluations[i] * k3
            + b_evaluations[i] * powers[i] * k4
        )
        % modulus
        for i in range(precision)
    ]
    ...
    o = [
        ...
        prove_low_degree(
            l_evaluations, G2, steps * 2, modulus, exclude_multiples_of=extension_factor
        ),
    ]
...
def verify_mimc_proof(inp, steps, round_constants, output, proof):
    ...
    assert verify_low_degree_proof(
        l_root, G2, fri_proof, steps * 2, modulus, exclude_multiples_of=extension_factor
    )
    ...
    for i, pos in enumerate(positions):
        ...
        assert (
            p_of_x
            - b_of_x * f.eval_poly_at(zeropoly2, x)
            - f.eval_poly_at(interpolant, x)
        ) % modulus == 0
